...
test-acceptance-wip: test-acceptance-setup ## Runs acceptance tests for WIP tagged scenarios
	...
	FEATURES_PATH=test/acceptance/features $(PYTHON_VENV_DIR)/bin/behavex -o $(TEST_ACCEPTANCE_OUTPUT_DIR) --no-capture --no-capture-stderr $(TEST_ACCEPTANCE_TAGS_ARG) $(EXTRA_BEHAVE_ARGS) -t "@wip" --stop
...
test-acceptance-wip-x: test-acceptance-setup ## Runs acceptance tests for WIP tagged scenarios
	...
	FEATURES_PATH=test/acceptance/features $(PYTHON_VENV_DIR)/bin/behavex -o $(TEST_ACCEPTANCE_OUTPUT_DIR) --no-capture --no-capture-stderr $(TEST_ACCEPTANCE_TAGS_ARG) $(EXTRA_BEHAVE_ARGS) -t "@wip" --parallel-processes $(TEST_ACCEPTANCE_PARALLEL)
