Feature: Use ServicesClass resources to manage RegisteredService resources
    ...
    # TODO(sadlerap): Once we have the agents more well-defined, we'll need to
    # assert that the registered services live on the main cluster, not the
    # worker cluster.
    Background:
    ...
    @disabled
    Scenario: Registered Services are removed when the corresponding Service Class is
        Given On Worker Cluster "worker", Resource is created
            """
            apiVersion: stable.example.com/v1
            kind: Backend
        ...
        And RegisteredService.primaza.io/$scenario_id-2 is available in cluster "main"
        When The resource serviceclasses.primaza.io/$scenario_id-serviceclass is deleted from the cluster "worker"
        Then The resource registeredservices.primaza.io/$scenario_id-1 is not available in cluster "main"
        Then The resource registeredservices.primaza.io/$scenario_id-2 is not available in cluster "main"
